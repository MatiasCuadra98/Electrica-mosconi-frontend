...
const DetailTable = ({state}) => {
    ...
    const sortMsgReceiveds = contact.MsgReceiveds.sort((a, b) => b.timestamp - b.timestamp);
    const lastMsgReceived = sortMsgReceiveds[0]
    console.log('ultimo mensaje recibido', lastMsgReceived);
    
    const sortMsgSents = contact.MsgSents.sort((a, b) => b.timestamp - b.timestamp);
              ...
              <tbody className="overflow-x-auto">
                      ...
                      <tr className={`${!state ? 'bg-white' : 'bg-stone-300'}`}>
                        <td className="pl-6 pr-4 py-2 text-center">
                          <StateMessagesIcons state={lastMsgReceived && lastMsgReceived.state} />
                        </td>
                        ...
                        <td className="px-[1.75rem] py-2 text-center w-5 h-5">
                          <SocialMediaIcons
                            socialMedia={contact.SocialMedium && contact.SocialMedium.name  ? contact.SocialMedium.name : null}
                          />
                        </td>
                        ...
                        <td className="pl-6 pr-4 py-2 text-center w-6 h-6 ">
                          <IconUser
                            name={lastMsgSent && lastMsgSent.User
